...
public class StarCraftMain {
	...
	public static void main(String[] args) {
		 ...
		 * 같은 인터페이스 or 부모 클래스를 공유하는 객체들이 서로 다른방식으로
		 * 동작 할 수 있는 특성
		 * 오버라이팅, 오버로딩, 업스케일, 다운스케일...
		...
		//타입이 부모타입이기 때문에 부모가 가지고있지않은 메소드 사용불가
		...
		StarUnit scv = new StarUnit("SCV", 5, 60, 5);
		//형변환 가능 체크(true/false);							//부모 -> 자식 (불가능)
		System.err.println(scv instanceof Zealot);			//부모객체는 자식객체로 형변환 할 수 없음
		StarUnit mStarUnit = marine01;						//자식 -> 부모 -> 자식(가능)
		System.out.println(marine01 instanceof Marine);		//부모 객체로 형변환 되었던 자식 객체는 가능
	}
